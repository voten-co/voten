<template>
    <section class="padding-bottom-10 flex1" id="submissions" :class="{'flex-center' : nothingFound}" v-infinite-scroll="loadMore" infinite-scroll-disabled="cantLoadMore">
...
<script>
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            ...
            if (Store.page.category.page === 0) {
                if (typeof app != "undefined") {
                    app.$Progress.start();
                }

                Promise.all([
                    Store.page.category.getSubmissions(to.query.sort, to.params.name),
                    Store.page.category.getCategory(to.params.name)
                ]).then(() => {
                    next(vm => {
                        vm.$Progress.finish();
                    });
        ...
        beforeRouteUpdate(to, from, next) {
            ...
            Promise.all([
                Store.page.category.getSubmissions(to.query.sort, to.params.name),
                Store.page.category.getCategory(to.params.name, false)
            ]).then(values => {
