<template>
	<div id="user-header">
		<div :style="{ background: coverBackground }"
		     class="profile-cover"
				...
				<div class="cols-flex">
					...
					<div class="user-header-middle">
						...
						<span class="inline-block">
							<i class="v-icon v-submissions"
							   aria-hidden="true"></i>{{ userStore.stats.submissions_count }}
						</span>

						<span class="inline-block">
							<i class="v-icon v-chat"
							   aria-hidden="true"></i>{{ userStore.stats.comments_count }}
						</span>

						<span class="inline-block">
							<i class="v-icon v-calendar"
							   aria-hidden="true"></i>Joined: {{ date }}
						</span>

						<a :href="userStore.info.website"
						   rel="nofollow"
						   target="_blank"
						   v-if="userStore.info.website"
						   class="inline-block">
							<i class="v-icon v-link"
							   aria-hidden="true"></i>{{ userStore.info.website }}
						</a>

						<span v-if="userStore.info.location"
						      class="inline-block">
							<i class="v-icon v-location"
							   aria-hidden="true"></i>{{ userStore.info.location }}
						</span>

						<span v-if="userStore.info.twitter "
						      class="inline-block">
							<a :href="'https://twitter.com/' + userStore.info.twitter"
							   rel="nofollow"
							   target="_blank">
								<i class="v-icon v-twitter"
								   aria-hidden="true"></i>{{ userStore.info.twitter }}
							</a>
						</span>
					</div>
		...
		<nav class="nav has-shadow user-select">
			<div class="container">
				<div class="nav-left">
					...
					<router-link :to="'/@' + $route.params.username + '/comments'"
					             class="nav-item is-tab"
					             active-class="is-active"
					             exact>
						Comments
				...
				<div class="channel-admin-btn">
					...
					<el-button type="danger"
					           size="mini"
					           round
							   v-if="$route.params.username != auth.username && !isGuest && isVotenAdministrator"
					           @click="banUser">
						Ban
					</el-button>
