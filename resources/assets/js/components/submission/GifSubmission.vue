<template>
    <div>
        <div v-if="showBigThumbnail" class="align-center relative pointer" @click="$emit('play-gif')">
            <img :src="submission.data.thumbnail_path" :alt="submission.title" class="big-thumbnail"/>
            ...
            <span class="submission-img-title">
                ...
                <div class="flex1" v-else>
					...
					<submission-footer :url="url" :comments="comments" :bookmarked="bookmarked" :submission="submission"
                                       v-if="!full"
                                       @bookmark="$emit('bookmark')" @report="$emit('report')" @hide="$emit('hide')"
                                       @nsfw="$emit('nsfw')" @sfw="$emit('sfw')" @destroy="$emit('destroy')"
                                       @approve="$emit('approve')" @disapprove="$emit('disapprove')"
                                       @removethumbnail="$emit('removethumbnail')" :upvoted="upvoted"
                                       :downvoted="downvoted" :points="points"
