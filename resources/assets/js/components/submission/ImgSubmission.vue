<template>
	<div>
		<div v-if="!isAlbum && showBigThumbnail">
			<img :src="submission.data.thumbnail_path" :alt="submission.title" @click="$emit('zoom')" class="big-thumbnail"/>
		</div>
		...
		<div class="link-list-info">
			<span class="submission-img-title">
				<a class="submisison-small-thumbnail" v-if="submission.data.thumbnail_path && !full">
					<!-- img -->
					<div v-if="showSmallThumbnail" class="small-thumbnail zoom-in" :style="thumbnail"
					@click="$emit('zoom')"></div>
				</a>
